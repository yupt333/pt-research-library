...
- 多数の `literature_ids` 指定、または非常に多数の文献に対するタグ取得は、SQLiteビルドの環境依存なバインド変数上限の影響を受ける。
- Phase 1では、チャンク分割や一時テーブル方式を実装しない。
- 上限超過時も、既存CSVを変更せず、新規CSVや一時ファイルを残さない原子的出力仕様を維持する。
...
- Phase 1では、スキーマ、文献、タグ、関連付け、使用履歴を含むSQLiteデータベース全体だけをバックアップする。
- SQLiteの `Connection.backup()` を使用し、WALなどの状態に依存するデータベースファイルの単純コピーは行わない。
- 設定ファイルは、設定ファイルを導入する段階でバックアップ対象を再検討する。
- CSVエクスポートはCSV出力機能で扱い、Phase 1のバックアップ対象には含めない。
...
- 最終公開には同じ出力先ディレクトリ内での `os.link()` によるハードリンク作成を使用し、`os.link()` の成功をバックアップ公開の成功確定点とする。
- 公開成功後は最終バックアップを削除しない。一時ファイルの削除はbest effortとし、削除に失敗しても実際に作成された最終バックアップのパスを返す。この場合、一時ファイルが残り、最終バックアップと同じinodeを指す可能性がある。
- ハードリンク非対応ファイルシステムでは `OSError` を伝播し、既存バックアップを変更せず安全に失敗する。
- バックアップ処理は元connectionを `commit`、`rollback`、`close` せず、元データベースを変更しない。
- 元connectionにアクティブなトランザクションがある場合は、バックアップを開始せず `ValueError` とする。
- 復元、圧縮、クラウド同期、古いバックアップの自動削除はPhase 1の対象外とする。
